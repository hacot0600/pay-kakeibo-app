    ...
    $('importAllFile').addEventListener('change', (e) => {
      ...
      const openSettingsPage = () => {
        document.querySelectorAll('.nav-btn').forEach((btn) => {
          const active = btn.dataset.page === 'settingsPage';
          btn.classList.toggle('active', active);
        });
        document.querySelectorAll('.page').forEach((page) => {
          page.classList.toggle('active', page.id === 'settingsPage');
        });
      };
      ...
      reader.onload = () => {
        ...
        try {
          ...
          if (result.ok) {
            toast('復元候補を読み込みました。下の「復元する」を押してください');
          } else {
      ...
      reader.onerror = () => {
        ...
        renderImportPreview({
          ok: false,
          errors: ['ファイルの読み込みに失敗しました。端末内に保存してから再度選択してください。']
        });
